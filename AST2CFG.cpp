...
AST2CFG::AST2CFG(SNode* root, CFG& cfg):          // Create constructor by passing Snode* and CFG&
	m_root(root), m_cfg(cfg)                           // m_root and m_cfg are the variables of Syntax2CFG
{
}
...
bool AST2CFG::execute()
{
        //printf("entering S2CFG");
	bool rc = false;
        //printf("working 41");
	clearCompoundBlocks();
        //printf("working 42");
	ChildrenOf.clear();
        //printf("working 43");
	SyntaxSimplifier(m_root).execute();       // reduceChildren(m_root), expandForLoops(m_root);
        //printf("working 44");
	...
	if (m_compoundBlocks.size() > 0)
	   ...
	   // printf("Entering the main CFG for Compound Blocks");
...
FlowPoint* AST2CFG::connectFlowPoints(FlowPoint* root,std::vector<FlowPoint*>& endFPs)
{	
...
/// Return value: A Flow Point which should be connected
///				  from previous ending flow points in parent scope.
FlowPoint* AST2CFG::connectFlowPointsInCompoundBlock(CompoundBlock* block,
													    std::vector<FlowPoint*>& endFPs)
{
	FlowPoint* startFP = NULL;
	endFPs.clear();
...
FlowPoint* AST2CFG::generateFlowPoints(SNode* root, FlowPoint* parent)
{	
	FlowPoint* ret = NULL;
...
FlowPoint* AST2CFG::generateStatementNodeFlowPoints(SNode* root, FlowPoint* parent)
{
	FlowPoint* ret = NULL;
	Block* b = NULL;
...
FlowPoint* AST2CFG::generateLoopNodeFlowPoints(SNode* root, FlowPoint* parent)
{
	FlowPoint* fp = NULL;
...
FlowPoint* AST2CFG::generateBranchNodeFlowPoints( SNode* root, FlowPoint* parent )
{
	FlowPoint* fp = NULL;
...
///         this sequence.
void AST2CFG::reduceExpressionBlocks()
{
...
Block* AST2CFG::newCompoundBlock(SNode* statement, FlowPoint* parent)
{
...
Block* AST2CFG::newExpressionBlock(SNode* statement, FlowPoint* parent)
{
...
void AST2CFG::concatVecs(std::vector<FlowPoint*>& a, const std::vector<FlowPoint*>& b)
{
...
void AST2CFG::clearCompoundBlocks()
{
...
/// into Expr. Block A.
void AST2CFG::mergeConsecutiveExpressionBlocks()
{
...
void AST2CFG::collectVariableAndConstantNames( SNode* root )
{
	if (root)
	{
		if (root->Type() == ID) 
