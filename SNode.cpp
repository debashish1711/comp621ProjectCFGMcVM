...
{
	m_children.reserve(node.m_childCount);    // So nodeData* should have m_childCount defined
...
} //how to compile c++ with only the files that are being modified
...
std::string SNode::Text() const
{
	...
	case CONDITION_IF_ELSE: 
		res = "if" + m_children[0]->Text() + " else ... end";
		break;
